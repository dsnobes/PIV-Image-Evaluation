function out=cntrd(im,mx,sz,intThresh,interactive)
...
mx=mx((mx(:,2) > 1.5*sz & mx(:,2) < nr-1.5*sz),:);
%ind=find(mx(:,1) > 1.5*sz & mx(:,1) < nc-1.5*sz);
mx=mx((mx(:,1) > 1.5*sz & mx(:,1) < nc-1.5*sz),:);
...
for i=1:nmx
    ...
    for j = 1:size(tmp,1)
        for k = 1:size(tmp,2)
            if tmp(j,k) >= intThresh
                totalI = totalI + tmp(j,k);
    ...
    if particleSize > 0
        peakI = max(max(tmp));
        pts=[pts,[mx(i,1)+xavg-r,mx(i,2)+yavg-r,totalI,rg,particleSize, peakI]'];
    end
